...
int main()
{
    ...
    int k=5; //改变以下循环的值
    for(int i=0 ; i<a[k] ; i++){
        kalmanFilterMain(data[k],mesure[k][i],pre_data[k]);

        cout<<"ID:"<<mesure[k][i].id<<"  inData.x:"<<mesure[k][i].x;
        cout<<"   inData.y:"<<mesure[k][i].y<<"    time:"<<mesure[k][i].time<<"=================="<<endl;
        cout<<"              x:"<<data[k].x.matrix[0][0]<<"  "<<data[k].x.matrix[1][0];
        cout<<"   y:"<<data[k].y.matrix[0][0]<<"  "<<data[k].y.matrix[1][0]<<endl;

        cout<<"              x:";
        cout<<pre_data[k].x.matrix[0][0]<<"  "<<pre_data[k].x.matrix[1][0];
        cout<<"   y:"<<pre_data[k].y.matrix[0][0]<<"  "<<pre_data[k].y.matrix[1][0]<<endl;
    }
